...
class MultiPoseDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    if self.opt.zjb:
      ...
      tl_heatmaps = np.zeros((self.num_classes, output_res, output_res), dtype=np.float32)
      br_heatmaps = np.zeros((self.num_classes, output_res, output_res), dtype=np.float32)
      ct_heatmaps = np.zeros((self.num_classes, output_res, output_res), dtype=np.float32)
      ...
      if rot != 0:
        ...
        reg_mask *= 0
        kps_mask *= 0
      ret = {'input': inp, 'ind': ind, #'hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'wh': wh,
             'hps': kps, 'hps_mask': kps_mask, 'tl_heatmaps': tl_heatmaps, 'br_heatmaps': br_heatmaps, 'ct_heatmaps': ct_heatmaps,
             'tl_regrs': tl_regrs, 'br_regrs': br_regrs, 'ct_regrs': ct_regrs, 'tl_tags': tl_tags, 'br_tags': br_tags, 'ct_tags': ct_tags, 'tag_masks':reg_mask}
      if self.opt.hm_hp:
